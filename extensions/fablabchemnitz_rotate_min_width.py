...
import sys
import inkex
import fablabchemnitz_rotate_helper
...
class RotateMinWidth(inkex.Effect):
    def effect(self):
        for node in self.svg.selected.values():
            min_width_angle = fablabchemnitz_rotate_helper.optimal_rotations(node)[0]
            if min_width_angle is not None:
                node.transform = fablabchemnitz_rotate_helper.rotate_matrix(node, min_width_angle) * node.transform
